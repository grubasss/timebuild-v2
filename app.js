...
<div class="row">
<b onclick="showProject('${p.id}')" style="cursor:pointer">${p.name}</b>
</div>
`).join("");
...
const entries = db.entries.filter(e=>e.project==id);
...
if(worker){
earned += Number(e.hours) * worker.rate;
}
